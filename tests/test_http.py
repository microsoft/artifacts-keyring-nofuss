...
@mock.patch("artifacts_keyring_nofuss._http._session_for_attempts")
class TestRequestRetries:
    ...
    def test_retries_on_timeout_then_succeeds(
        ...
        session = mock.Mock(spec=requests.Session)
        mock_session_for_attempts.return_value = session
        session.request.side_effect = [requests.Timeout("slow"), _response(200)]
    ...
    def test_does_not_retry_non_transient_exception(
        ...
        session = mock.Mock(spec=requests.Session)
        mock_session_for_attempts.return_value = session
        session.request.side_effect = requests.exceptions.InvalidURL("bad url")
    ...
    def test_does_not_retry_ssl_error(
        ...
        session = mock.Mock(spec=requests.Session)
        mock_session_for_attempts.return_value = session
        session.request.side_effect = requests.exceptions.SSLError("cert invalid")
...
class TestSessionConfiguration:
    def test_user_agent_falls_back_to_unknown(self) -> None:
        with mock.patch(
            "artifacts_keyring_nofuss._http.importlib.metadata.version",
            side_effect=importlib.metadata.PackageNotFoundError,
        ):
            assert _http._user_agent() == "artifacts-keyring-nofuss/unknown"
    ...
    def test_non_default_attempts_get_new_session(self) -> None:
        ...
        assert default_session is _http._SESSION
        assert (
            _http._session_for_attempts(_http._SESSION_ATTEMPTS + 1)
            is not _http._SESSION
        )
